...
- [What to review next](https://lab.abvx.xyz/planning/) — Read-only planning queue with status, priority, workflow-sync hints, and richer proof-loop readiness/evidence signals from SET batch planner.
- [Proof queue](https://lab.abvx.xyz/proof/) — Read-only proof-loop queue for blockers, review-ready tasks, and evidence quality.
